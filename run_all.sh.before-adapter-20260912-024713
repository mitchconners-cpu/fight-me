#!/bin/bash
# run-all.sh
# Master launcher: venv + deps + FastAPI (gateway_app:app) + ngrok + Telegram bot listener.
...
if [ ! -f "$PROJECT/local_twilio.py" ]; then
    echo "    ERROR: local_twilio.py not found at $PROJECT/local_twilio.py"
    exit 1
fi
...
if command -v ngrok >/dev/null 2>&1; then
    ngrok http "$PORT" --request-header-add="ngrok-skip-browser-warning:true" > "$PROJECT/ngrok.log" 2>&1 &
    NGROK_PID=$!
    sleep 3
